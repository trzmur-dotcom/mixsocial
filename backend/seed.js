...
async function seed() {
  ...
  for (const u of users) {
    ...
    await db.prepare('INSERT INTO users (username, email, password_hash, bio, is_demo) VALUES (?, ?, ?, ?, 1)')
      .run(u.username, u.email, hash, u.bio);
  }

  const stories = [
    {
      user_id: 1, cocktail_name: 'Classic Negroni',
      description: 'The iconic Italian aperitivo. Bitter, sweet, and perfect. 🍊',
      alcohol_types: ['gin', 'vermouth', 'campari'],
      ingredients: [
      ...
      difficulty: 'easy', prep_time: 3,
    },
    {
      user_id: 2, cocktail_name: 'Tom Collins',
      ...
      difficulty: 'easy', prep_time: 4,
    },
    {
      user_id: 3, cocktail_name: 'Old Fashioned',
      description: 'The timeless whiskey classic. Perfection in a glass 🥃',
      alcohol_types: ['whiskey', 'bourbon'],
      ingredients: [
      ...
      difficulty: 'medium', prep_time: 5,
    },
    {
      user_id: 1, cocktail_name: 'Espresso Martini',
      description: 'The perfect after-dinner cocktail. Wake up and party ☕✨',
      alcohol_types: ['vodka', 'coffee_liqueur'],
      ingredients: [
      ...
      difficulty: 'medium', prep_time: 7,
    },
    {
      user_id: 4, cocktail_name: 'Piña Colada',
      description: 'Tropical paradise in a glass 🌴🍍',
      alcohol_types: ['rum', 'coconut'],
      ingredients: [
      ...
      difficulty: 'easy', prep_time: 5,
    },
    {
      user_id: 2, cocktail_name: 'Gin Basil Smash',
      ...
      difficulty: 'medium', prep_time: 6,
    },
    {
      user_id: 3, cocktail_name: 'Whiskey Sour',
    ...
    {
      user_id: 4, cocktail_name: 'Mojito',
  ...
  for (const s of stories) {
    ...
    `).run(s.user_id, s.cocktail_name, s.description, JSON.stringify(s.alcohol_types),
            JSON.stringify(s.ingredients), JSON.stringify(s.instructions),
            s.food_pairing, s.difficulty, s.prep_time);
  }

  // Some saved recipes for demo user
  await db.prepare('INSERT INTO saved_recipes (user_id, story_id, rating) VALUES (1, 2, 8)').run();
  await db.prepare('INSERT INTO saved_recipes (user_id, story_id, rating) VALUES (1, 3, 9)').run();
  await db.prepare('INSERT INTO saved_recipes (user_id, story_id, rating) VALUES (1, 5, 7)').run();

  // Bar for demo user
  for (const t of ['gin', 'whiskey', 'vodka']) {
    await db.prepare('INSERT INTO user_bar (user_id, alcohol_type) VALUES (1, ?)').run(t);
  }
